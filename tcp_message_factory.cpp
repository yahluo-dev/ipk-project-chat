...
std::unique_ptr<Message> TCPMessageFactory::create(const std::string &message)
{
  std::smatch matches;
  if (!std::regex_match(message, std::regex("[^\r\n]+\r\n")))
  {
    return std::make_unique<UnknownMessage>(CODE_UNKNOWN);
  }
  ...
  if (std::regex_match(message, matches, msg_regex))
  {
    ...
    auto parsed_message = std::make_unique<MsgMessage>(0, display_name, message_contents);
  ...
  else if (std::regex_match(message, matches, err_regex))
  {
    std::string display_name = matches[1];
    std::string message_contents = matches[2];
    auto parsed_message = std::make_unique<ErrMessage>(0, display_name, message_contents);
    return parsed_message;
  }
  else if (std::regex_match(message, matches, reply_regex))
  {
    ...
    std::string message_contents = matches[2];
    auto parsed_message = std::make_unique<ReplyMessage>(0,
                                           (result == "OK" ? 1 : 0),
                                           0, message_contents);
    return parsed_message;
  }
  ...
  {
    auto parsed_message = std::make_unique<ByeMessage>(0);
    return parsed_message;
  }
  else
  {
    auto parsed_message = std::make_unique<UnknownMessage>(CODE_UNKNOWN);
    return parsed_message;
  }
}
